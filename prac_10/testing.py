...
def format_phrase_as_sentence(phrase):
    """
    Format a phrase passed in as a sentence (starting with capital, ending with full stop).
    >>> format_phrase_as_sentence('hello')
    'Hello.'
    >>> format_phrase_as_sentence('It is an ex parrot.')
    'It is an ex parrot.'
    >>> format_phrase_as_sentence('this can be a sentence if punctuated correctly')
    'This can be a sentence if punctuated correctly.'
    """
    if phrase[-1] == ".":
        phrase = phrase[:-1]  # Omit final character if it is already a full stop
...
def run_tests():
    ...
    assert test_car.fuel == 10, "Car does not set fuel correctly from value passed in"
    test_car = Car()
    assert test_car.fuel == 0, "Car does not set fuel correctly from default"
